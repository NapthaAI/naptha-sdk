...
def run_mas(multi_agent_service, mas_run) -> None:
    loop = asyncio.get_event_loop()
    mas_engine = MASEngine(multi_agent_service, mas_run)
    loop.run_until_complete(mas_engine.start_run())
...
class MASEngine:
    def __init__(self, multi_agent_service, mas_run):
        ...
        logger.info("KKKKKKK", self.mas_run)
    ...
    async def start_run(self):
        ...
        self.mas_run.status = "running"
        await self.mas.orchestrator_node.update_mas_run(module_run=self.mas_run)

        logger.info(f"Checking user: {self.consumer}")
        consumer = await self.mas.orchestrator_node.check_user(user_input=self.consumer)
        if consumer["is_registered"] == True:
            ...
            consumer = await self.mas.orchestrator_node.register_user(user_input=consumer)
            logger.info(f"User registered: {consumer}.")

        logger.info(f"Running multi agent service on orchestrator node {self.mas.orchestrator_node.node_url}: {self.mas_run}")
        mas_run = await self.mas.orchestrator_node.run_task(module_run_input=self.mas_run)
        logger.info(f"Created multi agent service run on orchestrator node {self.mas.orchestrator_node.node_url}: {mas_run}")

        while True:
            mas_run = await self.mas.orchestrator_node.check_task(mas_run)
            logger.info(mas_run.status)  
            await self.mas.orchestrator_node.update_task_run(module_run=mas_run)
