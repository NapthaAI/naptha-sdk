...
from naptha_sdk.package_manager import add_files_to_package, add_dependency_to_pyproject, create_poetry_package, extract_packages, publish_ipfs_package, transform_code_agent
...
class Naptha:
    ...
    async def publish(self):
        for agent in self.agents:
            ...
            agent_code = inspect.getsource(agent.fn)
            agent_code = transform_code_agent(agent_code)
            # packages = extract_packages(agent_code)
            # logger.info(f"PACKAGES: {packages}")
            create_poetry_package(agent.name)
            # add_dependency_to_pyproject(agent.name, packages)
